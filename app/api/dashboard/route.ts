// GET /api/dashboard — Dados do dashboard com suporte a filtros
import { NextRequest, NextResponse } from 'next/server'
import { Prisma } from '@prisma/client'
import { prisma } from '@/lib/prisma'
import { getAuthSession, unauthorized } from '@/lib/api-helpers'

function calcularPeriodo(periodo?: string, dataInicio?: string, dataFim?: string) {
  const hoje = new Date()
  let inicio: Date
  let fim: Date = hoje
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { inicio, fim } = calcularPeriodo(periodo, dataInicioParam || undefined, dataFimParam || undefined)

    // Base where clause for cobranças (período)
    const cobrancaWhereBase: any = {
      deletedAt: null,
      createdAt: { gte: inicio, lte: fim },
    }

    // If rotaId filter, need to join via cliente
    if (rotaId) {
      cobrancaWhereBase.cliente = { rotaId }
    }

    // Período anterior para variação
    const periodoAnteriorInicio = new Date(inicio)
    periodoAnteriorInicio.setMonth(periodoAnteriorInicio.getMonth() - (periodo === 'trimestre' ? 3 : periodo === 'semestre' ? 6 : 1))

    const cobrancaWhereAnterior: any = {
      deletedAt: null,
      createdAt: { gte: periodoAnteriorInicio, lt: inicio },
    }
    if (rotaId) {
      cobrancaWhereAnterior.cliente = { rotaId }
    }

    // Cliente where for rota filter
    const clienteWhere: any = { status: 'Ativo', deletedAt: null }
    if (rotaId) clienteWhere.rotaId = rotaId

    // Cobranças por status (período filtrado)
    ...
    // Calcular range para gráfico (últimos 6 meses a partir do fim do período)
    const seisMesesAtras = new Date(fim)
    seisMesesAtras.setMonth(seisMesesAtras.getMonth() - 5)
    seisMesesAtras.setDate(1)

    const receitaWhere6Meses: any = {
      deletedAt: null,
      createdAt: { gte: seisMesesAtras },
    }
    if (rotaId) receitaWhere6Meses.cliente = { rotaId }
    ...
    const receitaPeriodo = cobrancasPeriodo._sum.valorRecebido ?? 0
    const receitaAnterior = cobrancasPeriodoAnterior._sum.valorRecebido ?? 0
    ...
    // Serializar cobrancasRecentes para o cliente
    ...
    return NextResponse.json({
      ...
      totalCobrancasMes: cobrancasPeriodo._count,
